...
const NotificationComponent = ({username, type}) => {
    ...
    return (
        <li>
            <div className={`w-full lg:w-1/3 xl:w-1/3 flex items-center ${bgColor(type)} text-white text-sm font-bold px-4 py-3`} role="alert">
                ...
                <strong>{username}</strong><p className="ml-1">{message(type)}</p>
            </div><br/>
        </li>
    )
}
...
const Main = () => {
    ...
    useEffect(() => {
        async function fetchUserCheck(){
            try{
                const { data:matchData } = await axios.get(`/user/check?id=${query.get("id")}&username=${query.get("username")}`);
                if(!matchData.error !== null)
                    setError(matchData);
            } catch(error){
                console.log(error);
            }
    ...
    return (
        ...
        <main className="w-full bg-gray-100 h-screen flex items-center justify-center">
            <div className="flex flex-col h-3/4 w-3/4 p-6 rounded shadow-2xl bg-gray-200">
                <ul id="chat-display" className="h-full w-full overflow-y-scroll">
                {
                    chat.map((v, k) => {
                ...
                <div className="h-20 w-full p-2">
                    <form id="chat-input" onSubmit={e => HandleSubmit(e)} className="h-full flex items-center justify-center">
                        ...
                        <button type="submit" className="h-full w-16 bg-gray-300">Send</button>
                    </form>
                </div>
            </div>
        </main>
